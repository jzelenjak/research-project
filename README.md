...
**Example use case**: after running SAGE on CPTC-2017 (or CPTC-2018) before and after merging sinks states, you want to compare the generated attack graphs side-by-side in terms of the size and complexity (and other interesting notes). You can use various filters in the script (by commenting out or uncommenting lines) to filter only the attack graphs you are interested in (e.g. to get only the attack graphs that changed their complexity).
...
**Example use case**: you want to compute statistics on a single directory with attack graphs, for example, before merging sinks or after merging sinks.

### stats-ff.sh

**Description**: this script is used to get the number of red, blue and white states in the final S-PDFA generated by FlexFringe. This script has only been used when formulating the hypothesis and checking how the final S-PDFA has been affected by merging sinks.
...
**Description**: this script computes the statistics on the nodes of the attack graphs in the specified directory (total number of nodes, number of unique nodes, total number of sinks, number of unique sinks). This script has only been used when formulating the hypothesis and checking how the node count of the attack graphs has been affected by merging sinks.
...
**Description**: this script is primarily used inside the `./stats-ags.sh` script to compute the regression line for classification of the attack graphs as complex and non-complex. It is not very useful as a stand-alone script.
